...
A system may have a peak of 18000 services tasks/ second for 30 min, when that average the rest of the day is about 800 services tasks/seconds. 
...
The cluster will be able to absorb the peak in 2 hours. And it will be 1800/1000 smaller.
...
if the platform must absorb the peak, then the throughput in the peak is your goal.
...
increase the platform resource, to ensure all execution run as fast as possible.


## Replication factor

A replication factor determine the number of broker update in real time. With a replication factor = 3, 3 database are updated in real time.
...
If this solution is acceptable? To reduce the risk, multiple data center can be used, and a broker place on each datacenter.
If a data center is down, it's acceptable.
The risk to have two data center down at the same time can be considered as extremery rare, and acceptable for a customer, when another can decide to not take this risk.
...
Using multiple region and replication factor = 3 can be reasonable too. With 2 regions, this implie 2 brokers will be on the same region, so if this region die, cluster is pause.
A new broker can be start in the survivor region to reach the quorum, or 3 regions can be used, one broker in each region.
...
A Zeebe server is not very consuming in CPU, or in memory, but this aspect must be take into account in the load test.
...
* the path on each process: one process may have decision gateway, and one path contains 3 services tasks, the second 20 services task. Does
Does processes instance follow the 3 services task path? 20% on this path, 80% on the second one? 

* Do you have an iteration? How many iterations in average?  


* Data are essential. More data you have, more pressure you send to the Zeebe Engine to manipulate and store data.
...
If the execution varies between 1 and 5 seconds, it will wait for the longest execution to ask again for the next job: it will wait 5 seconds. So, the pod may have a low CPU time execution.
...
Choose a reasonable time to run a test. Calculate the time to execute one process instance: for example, additionning the time of all service task, a process instance needs 4 minutes.
...
To run a first load test, a technical estimation can be done.
...
The number of partitions determine the number of Leader. Followers are the replica factor -1.
...
If you want this certitude, you must set 
```
...
In terms of throughput, this situation will reduce the CPU on Leader broker (the have one follower less) and you will have some pods under used.
...
Which mean a partition can handle
```
500/3=166 services task per second
```
This approach implies to estimate first the number of service task per second.
To do that, start from the goal in terms of process instance, and look in processes to estimate the path, and dertermine the number of service task per process instance. Consider a send message as a service task.
...
But if there is now a trhoughput of 10 service tasks every minute, 1 thread is not enought: one thread can handle 60/8=7.5 tasks per minutes.
...
if the worker need 8 seconds and consult CPU during this 8 seconds (it merge images in a PDF), maybe a worker can handle 80 to 100 threads.
...
With Zeebe, a partition is a complete server. So, adding a partition increase the throughput, But different factor comes:
* adding partitions increase the work for the zeebe gateway, to search for job for a workers. Maybe this must be added
* Adding partitions implie to add a pod. Does the physical node can handle that? Does a node is necessary?

Adding partition is maybe not the solution: may a worker is under estimated, and the throughput is not reach because there is a lot of jobs waiting in a task?

Monitoring the cluster during the load test is mandatory. Looking Operate to detect any jobs in suspend is mandatory too.

During the load test, this metrics must be checked.
...
* an application ask to create a process instance, It connects the Zeebe Gateway, which choose a partition, then connect the Leader of the partition
* the leader register the order in the stream, send the information to followers and wait the acknoledge
* when the leader get the acknowledge, it executes the order (create the process instance), return the informqtion to the client, and send the status to followers
...
* a worker connect the Zeebe Gateway to get jobs related to a topic
...
* Zeebe gateway return the list to worker
...
Four throughput nust be checked:
...
Zeebe export data in a Raw index, then Operate import the data in the Operate index.
...
During one minute, information in Operate should change (importer finish to import the last information), but if information still change after this time, operate importer is too slow and must be scale.
...
* Leader take time to contact followers, especially when followers are on a different region
* disk is too slow (check the IO metrics)
* Elastic search exporter is too slow (check the exporter pointer)

A simple solution consist of increasing the number of partition. Start by adding 5% of partitions if you have a 5% backpressure.
...
Zeebe do a very quick jobs on the creation, reaching 1000 PI/seconds easily. 

Check the tool who creates the process instances or increase the number of partitions. Check the GRPC latency on creation.
