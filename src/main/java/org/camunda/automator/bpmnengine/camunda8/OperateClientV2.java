...
public class OperateClientV2 implements OperateClientInt {
    ...
    public void connectOperate(StringBuilder analysis) throws AutomatorException {
        ...
        if (engineCamunda8.getCamundaClient() == null) {
            analysis.append("No camundaClient connection, ");
            return;
        ...
        try {
            ...
            operateClient = new CamundaOperateClientV2(engineCamunda8.getCamundaClient());
    ...
    public List<String> activateServiceTasks(String processInstanceId, String serviceTaskId, String topic, int maxResult)
        ...
        try {
            ...
            processInstanceQuery.setSort(List.of(new Sort("StartDate", SortOrder.ASC)));
    ...
    public List<BpmnEngine.TaskDescription> searchTasksByProcessInstanceId(String processInstanceId, String filterTaskId, int maxResult)
        ...
        while (retry < 3) {
            ...
            try {
                ...
                flowNodeQuery.setSort(List.of(new Sort("startDate", SortOrder.ASC)));
    ...
    public Map<String, Object> getVariables(String processInstanceId) throws AutomatorException {
        try {
            ...
            variableQuery.setSort(List.of(new Sort("name", SortOrder.ASC)));
            ...
            // transform the value : the API return everything as a String.
            ObjectMapper objectMapper = new ObjectMapper();
            listVariables.forEach(v -> {
                Object typedValue;
                try {
                    // "344" -> Integer(344), "554.23" -> Double(554.23),
                    // "\"color\"" -> "color", "[1,2]" -> List, "{\"a\":1}" -> Map, etc.
                    typedValue = objectMapper.readValue(v.getValue(), Object.class);
                } catch (JsonProcessingException e) {
                    // fallback if it's not valid JSON
                    typedValue = v.getValue();
                }

                variables.put(v.getName(), typedValue);
            });
    ...
    public long countNumberOfProcessInstancesEnded(String processId, Date startDate, Date endDate)
        ...
        try {
            ...
            do {
                ...
                SearchQuery searchQuery = queryBuilder.build();
                searchQuery.setSize(SEARCH_MAX_SIZE);
                searchQuery.setSort(List.of(new Sort("key", SortOrder.ASC)));
                // Operate client does not support the multithreading very well (execption in jackson library)
                synchronized (this) {
                    searchResult = operateClient.searchProcessInstanceResults(searchQuery);
                }
    ...
    public long countNumberOfTasks(String processId, String taskId) throws AutomatorException {
        ...
        try {
            ...
            do {
                ...
                SearchQuery searchQuery = queryBuilder.build();
                searchQuery.setSize(SEARCH_MAX_SIZE);
                searchQuery.setSort(List.of(new Sort("StartDate", SortOrder.ASC)));
